...
def move_to_basket(request, item_id):
    ...
    if size:
        if item_id in basket:
            if size in basket[item_id]['sizes']:
                basket[item_id]['sizes'][size] += quantity
                messages.success(request, f'Updated size {size.upper()} {product.name} quantity to {basket[item_id]["sizes"][size]}')
            else:
                basket[item_id]['sizes'][size] = quantity
...
def adjust_basket(request, item_id):
    ...
    if size:
        if quantity > 0:
            ...
            del basket[item_id]['sizes'][size]
            if not basket[item_id]['sizes']:
                basket.pop(item_id)
